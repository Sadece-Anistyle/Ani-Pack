 ...
 */
function anipack(client, activities, interval = 120000, logActivity = true, webhook = null) {
  ...
  async function sendWebhook(activity, presenceData) {
    ...
    try {
      ...
      const webhookBody = {
        username: "anipack Status Updater",
        embeds: [{
  ...
  aniguncelle();
  setInterval(aniguncelle, interval);
}
 ...
 * @param {string|Object} webhook
 */
function ani(client, webhook = null) {
  if (!client || !client.user) {
    throw new Error("Geçerli bir Discord client nesnesi sağlamalısın.");
  }
  ...
  const statuses = [
    { name: '##anidesign', type: ActivityType.Playing },
    { name: '##anidesign', type: ActivityType.Listening },
    { name: '##anidesign', type: ActivityType.Streaming, url: 'https://twitch.tv/anistyle' },
    { name: '##anidesign', type: ActivityType.Watching }
  ];
